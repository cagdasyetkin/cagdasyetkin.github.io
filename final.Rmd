...
Ancient city names are easily available online. We will join their dataframe with our text. Our objective is to see how many times these cities are mentioned in our text and visualize them.
...
We can create a data structure to understand the co occurance of these cities with words like "to" and "from". It can give us a **naive** estimate of goods and services coming in and out. "to" stands for import, "from" stands for export, summation of "in"+"at" stands for inventory.
...
ggmap(b) + geom_point(data = cities_in_text, 
           ...
           title="City Locations and their Trading Power", color = "Type") +
...
*Kuliya wakes up early morning with the sound of black >> donkeys. They are on the way to Ulama.*
...
We see that we have quite many positive sentiment as well as negatives. And the financial analysis lexicon is giving accurate matches. For example seizing someone because of his debts.
...
oare_lda <- LDA(oare_dtm, k = 2, control = list(seed = 1234))

#per-topic-per-word probabilities
oare_topics <- tidy(oare_lda, matrix = "beta")

top_terms <- oare_topics %>%
  group_by(topic) %>%
  top_n(5, beta) %>%
  ungroup() %>%
  arrange(topic, -beta)
